...
def configure_provider(use_llm: bool):
    ...
    # Use OpenAI if configured, else Ollama
    if settings.PROVIDER == "openai" and settings.OPENAI_API_KEY:
        ...
        Settings.llm = Ollama(
            ...
            temperature=0.2,
            request_timeout=600.0,       # allow up to 10 min for cold start
            keep_alive="10m",
        )
...
def main():
    ...
    if Settings.llm is None:
        ...
        print("\n(LLM is disabled; run without --no-llm once you have a local LLM configured.)")
        return

    # Full RAG query with system prompt
    system_prefix = (
        "You are a helpful cafe menu assistant. "
        "Always answer in full sentences, not just numbers. "
        "Clearly list item names with their prices. "
        "If nothing matches, say it's not available."
    )

    query_engine = index.as_query_engine(similarity_top_k=args.k, response_mode="compact")
    try:
